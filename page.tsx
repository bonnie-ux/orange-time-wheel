...
function runSelfTests(): void {
  ...
  console.assert(
    Math.round(segments[1]!.end) === 360,
    "Expected last segment to end at 360"
  );

  const code = createCouponCode("ABC123");
  console.assert(
    code.startsWith("OT-123-"),
    "Expected coupon code suffix to use last 3 chars"
  );

  const forced = resolvePrize([{ name: "今天免費", weight: 1 }]);
  console.assert(
    forced.name === "今天免費",
    "Expected forced prize to resolve correctly"
  );

  const path = arc(100, 100, 50, 0, 90);
  console.assert(
    path.startsWith("M 100 100 L"),
    "Expected SVG arc path to be valid"
  );
    ...
    >
      <div className="mb-6 flex w-full max-w-3xl items-center gap-3">
        ...
        <div>
          ...
          <div className="text-sm" style={{ color: BRAND.softText }}>
            品牌活動正式版
          </div>
        </div>
      </div>
      ...
      <div
        className="mb-4 rounded-full px-5 py-2 text-base font-semibold"
        style={{ backgroundColor: "#F6DEC0", color: BRAND.primaryDark }}
      >
        ✨ 每日限抽一次
      </div>

      <h2
        className="mb-2 text-center text-5xl font-bold"
      ...
      <div className="mt-6 w-full max-w-3xl rounded-[36px] bg-[#f2eeea] p-6 shadow-[0_6px_20px_rgba(0,0,0,0.06)]">
        <div
          className="mb-3 text-left text-2xl font-bold"
          style={{ color: BRAND.dark }}
        >
          抽獎識別碼（例如手機末3碼 / 會員編號）
        </div>
        <input
          ...
          className="h-16 w-full rounded-[28px] border border-[#D9B27C] bg-white px-6 text-center text-2xl font-bold text-[#2B1A12] placeholder:text-[#6B7280] focus:outline-none"
        />
      </div>
      ...
      <AnimatePresence>
        {result ? (
          ...
          >
            <div className="mb-2 text-sm tracking-[0.2em] text-orange-600">
              抽獎結果
            </div>
            <div
            ...
            <div className="text-lg font-semibold" style={{ color: BRAND.dark }}>
              兌換序號
            </div>
            <div
              className="mt-2 rounded-2xl bg-[#FFF5E7] px-4 py-3 font-mono text-2xl font-bold tracking-wider"
      ...
      <div className="mt-6 w-full max-w-3xl rounded-[32px] bg-white p-6 shadow">
        <div
          className="mb-3 text-left text-xl font-semibold"
          style={{ color: BRAND.dark }}
        >
          今日獎項
        </div>
        <div className="flex flex-wrap gap-3">
          {PRIZES.map((item) => (
            <div
              ...
              style={{ backgroundColor: "#FFF2DD", color: BRAND.primaryDark }}
            >
              {item.name}
            </div>
          ))}
